...
def generate_text_summary(moderation_results: dict[str, float]) -> str:
    ...
    harmful_content = [label for label, score in moderation_results.items() if score >= thresholds[label]]
    if harmful_content:
        summary = "The text contains harmful content: " + ", ".join(harmful_content)
    else:
        summary = "The text content is safe and does not contain toxic or harmful elements."
...
def generate_image_summary(moderation_result: List[Dict[str, float]]) -> str:
    ...
    summary = f"The image contains harmful content: {', '.join(harmful_content)}" if harmful_content else ("The image "
                                                                                                           "is safe.")
    return summary
